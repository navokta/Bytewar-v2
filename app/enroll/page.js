...
export default function WowEnrollForm() {
  ...
  const [participationType, setParticipationType] = useState("team"); // 'solo' or 'team'
  ...
  useEffect(() => {
    ...
    if (savedData) {
      try {
        ...
        // Determine participation type from saved data
  ...
  const handleSubmit = async (e) => {
    ...
    // Prepare submission data based on participation type
    const submissionData = {
      ...formData,
      members: participationType === "solo" 
        ? [formData.members[0]] // Only include the first member for solo
        : formData.members.slice(0, teamSize) // Include selected team size for team
    };
            ...
            <div className="relative z-10 p-6 md:p-8 lg:p-10">
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-700 shadow-lg">
                  ...
                  {/* Team Configuration (only shown for team participation) */}
                  {participationType === "team" && (
                    <>
                      {/* Team Size Selector */}
                      <div className="mb-8">
                  ...
                  {/* Solo Participant Fields (only shown for solo participation) */}
                  {participationType === "solo" && (
                    <div className="space-y-4">
                      <div className="p-5 bg-gray-700/50 rounded-xl border border-gray-600 transition-all duration-300 hover:bg-gray-700/70">
                        <h5 className="font-medium text-white mb-4 flex items-center gap-3">
                          <div className="w-7 h-7 rounded-full bg-gradient-to-br from-purple-600 to-blue-600 flex items-center justify-center text-xs font-bold">
                            1
                          </div>
